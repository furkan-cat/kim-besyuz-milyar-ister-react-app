import React from "react";
import "./Prize.scss";

function Prize() {
  return (
    <div className="prize-wrapper">
      <div className="prize__list">
        <div>
          <div>15</div>
          <div className="money">$ 1 Million</div>
        </div>
        <div>
          <div>14</div>
          <div className="money">$ 500.000</div>
        </div>
        <div>
          <div>13</div>
          <div className="money">$ 250.000</div>
        </div>
        <div>
          <div>12</div>
          <div className="money">$ 64.000</div>
        </div>
        <div>
          <div>11</div>
          <div className="money">$ 32.000</div>
        </div>
        <div>
          <div>10</div>
          <div className="money">$ 16.000</div>
        </div>
        <div>
          <div>09</div>
          <div className="money">$ 8.000</div>
        </div>
        <div>
          <div >08</div>
          <div className="money">$ 4.000</div>
        </div>
        <div>
          <div>07</div>
          <div className="money">$ 2.000</div>
        </div>
        <div>
          <div>06</div>
          <div className="money">$ 1.000</div>
        </div>
        <div>
          <div>05</div>
          <div className="money">$ 500</div>
        </div>
        <div>
          <div>04</div>
          <div className="money">$ 300</div>
        </div>
        <div>
          <div>03</div>
          <div className="money">$ 200</div>
        </div>
        <div>
          <div>02</div>
          <div className="money">$ 100</div>
        </div>
        <div>
          <div>01</div>
          <div className="money">$ 100</div>
        </div>
      </div>
    </div>
  );
}

export default Prize;
